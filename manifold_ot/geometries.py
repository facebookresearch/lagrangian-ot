...
from manifold_ot import metrics, geodesics, lagrangian_potentials, splines, spline_amortizer
...
def get(name, geometry_kwargs):
    if name == 'sq_euclidean':
        ...
        bounds = (jnp.array((xbounds[0], ybounds[0])),
                  jnp.array((xbounds[1], ybounds[1])))
        return MetricManifold(
            xbounds = xbounds, ybounds = ybounds, bounds = bounds,
            distance_mode=DistanceModes.LAGRANGIAN,
            metric_initializer_fn=metrics.EuclideanMetric,
...
class MetricManifold(GeometryBase):
    ...
    def path(self, x, y, num_points=20):
        assert x.ndim == 1 and y.ndim == 1
        init_spline_params = jax.lax.stop_gradient(
            self.predict_spline_params(x, y))
        ...
        if initializing:
            ...
            xs = splines.compute_spline(
                x=x, y=y, basis=self.spline_amortizer.basis,
                params=init_spline_params, ts=ts)
            E = self.curve_energy(xs)
            return xs

        out = self.spline_geodesic_solver.solve(
            self.curve_energy, x, y,
    ...
    def energy_at_point(self, x, v):
        M = self.metric(x)
        if self.distance_mode == DistanceModes.GEODESIC or self.distance_mode == DistanceModes.SQUARED_GEODESIC:
    ...
    def add_plot_background(self, params, axs, xlims, ylims=None, alpha=1.0):
        ...
        if issubclass(self.metric_initializer_fn, metrics.ScarvelisMetric) \
            ...
            if not hasattr(self, 'lagrangian_potential_vmap_jit'):
                ...
                def lagrangian_potential_vmap(params, x):
                    return self.apply({'params': params}, x,
                                      method=self.lagrangian_potential)
